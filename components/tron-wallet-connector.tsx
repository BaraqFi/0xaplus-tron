...
export default function TronWalletConnector() {
  ...
  const { address, connect, disconnect: wcDisconnect, error: wcError } = useTronWalletConnect()
    ...
    <>
      ...
      <div className="w-full max-w-md mx-auto space-y-6 pt-4">
        <div className="text-center mb-8">
          ...
          {!connectedWallet && (
            <>
              ...
              <Button
                onClick={connect}
                className="w-full h-14 text-lg font-semibold bg-green-600 hover:bg-green-700 text-white shadow-lg mb-4"
              >
                {address ? "Connected via WalletConnect" : "Connect Trust Wallet (WalletConnect)"}
              </Button>
            </>
          )}
        </div>
        ...
        {wcError && (
          <div className="flex items-center gap-2 p-3 bg-red-500/20 border border-red-500/30 rounded-lg">
            <AlertCircle className="w-4 h-4 text-red-400" />
            <span className="text-white text-sm">{wcError}</span>
          </div>
        )}
